...
def relu_backward(dA, cache):
    ...
    # print("888888888", Z.shape)
    # print(type(Z))
    # print(Z)
    if Z.shape[0]==1:
        dZ=[0 for _ in range(Z.shape[1])]
...
def compute_cost(AL, Y, i):
    ...
    cost/=(i+1)**0.3
    cost=cost+max(0,random.gauss(0,2))
    assert(cost.shape == ())
...
def load_data():
    ...
    train_y=np.array(0)
    train_x=np.array(np.array([0,0,0,0]))

    for i in range(count): 
        state, current, dumping_time=time_state(time_list, i*1)
        v=[]
        for _ in range(factor):
            v.append(load_sensor(state, current, dumping_time, capacities["max"][_], capacities["min"][_]))
            # print("==========", value)
        value=np.array(v)
        train_x=np.vstack((train_x, value))
        guess=np.sum(value)/4
        final_value=np.random.normal(guess, 0.3)
        final_value=max(0,max(0.9*guess, min(1.1*guess, final_value)))
        train_y=np.append(train_y, final_value)
